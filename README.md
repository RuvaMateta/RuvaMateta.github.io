# RUVARASHE MATETA-Biotechnology & Bioinformatics

Welcome to my professional portfolio.
I am a biotechnology and bioinformatics student currently engaged in a **remote internship** focused on **genetic variation** and the **evolution of tomato flavor and nutrition**.  
This portfolio highlights my skills, applied research projects, and growing expertise in **computational genetics** and **data‑driven biotechnology**.

Alongside my academic work, I am part of the **Aspire Leaders Program (APL) Cohort 2** and actively involved in the **Mastuo Lab Data Science Program**, expanding my expertise in leadership, data science, and applied research.

## 🎓 Education
- **Bachelor’s studies in Biotechnology & Bioinformatics**  
  Focused on genetics, computational biology, and applied biotechnology
- **O’Level & A’Level ZIMSEC Certificates**  
 Strong foundation in sciences, mathematics, and applied subjects
- **Aspire Leaders Program (APL) Cohort 2**  
 Global leadership and professional development program.  
- **Mastuo Lab Data Science Studies**  
 Training in applied data science and computational methods.  
- **Certificate of Participation — Genomac Institute Inc.**  
  Exposure to genomics and applied biotechnology practices.
- **Certificates in Art and Chess**
  Recognition of creative and analytical skills beyong science.
  
  ---
  ## 🔬 About Me
- 🌍 Remote internship: genetic variation and tomato flavor/nutrition research  
- 🧬 Experience in bioinformatics pipelines and variant calling  
- 💻 Skilled in **Python programming** for data science and bioinformatics  
- 📊 Data analysis, visualization, and machine learning applications  
- 🐧 Comfortable working in Linux/WSL and GitHub for collaboration

  ## 🚀 Featured Projects
- **Tomato SNP/INDEL Pipeline**  
  End‑to‑end workflow for variant calling using FastQC, fastp, BWA, Samtools, and bcftools.
---
## 🛠️ Skills
- Bioinformatics workflows & variant calling  
- Python programming for data science & bioinformatics  
- Machine learning (clustering, segmentation)  
- Data analysis & visualization  
- Linux/WSL command‑line navigation  
- Git/GitHub collaboration  
- Leadership & data science (APL & Mastuo Lab)
- Biotechnology techniques (genomics, molecular biology, applied biotechnology)  
- Applied biotechnology (Genomac Institute Inc.)

  ---

## 📫 Contact
- 💼 LinkedIn: [www.linkedin.com/in/ruvarashe-mateta-993ba3316]
- 📱 Phone: [+263787092223] 
- 📧 Email: matetaruvarashe@gmail.com 
- 🐙 GitHub: [https://ruvamateta.github.io/] 
